...
from ..cache.types import WorkerInfo, CachedTask, CachedResult
from .model import (
    CreateTaskRequest,
    CreateTaskResponse,
...
async def monitor_workers(
    ...
    while True:
        try:
            ...
            print(f"Error in monitor_workers: {e}")
...
async def list_tasks(request: Request) -> ListTasksResponse:
    ...
    for worker in workers.values():
        for method in worker["capabilities"]["methods"]:
            capabilities[method] = capabilities.get(method, 0) + max(
                0, worker["capabilities"]["cpu"]
            )
...
async def get_task_status(
    ...
    if result_cache.exists(f"result:{task_id}"):
        [worker_id, completed_at, execution_time, result] = result_cache.hmget(
            f"result:{task_id}", "worker_id", "completed_at", "execution_time", "result"
        )
    ...
    [created_at, retry_count] = task_queue.hmget(
        f"task:{task_id}", "created_at", "retry_count"
    )
...
async def worker_heartbeat(
    ...
    if logging.getLogger().level <= logging.INFO:
        ...
        if new != old:
            ...
            for task_id in new:
                logging.info(f"Worker {worker_id} started task {task_id}")
            for task_id in old:
                logging.info(f"Worker {worker_id} stopped task {task_id}")
...
async def worker_get_task(
    ...
    task_queue.hset(f"task:{task_id}", "lease_expires_at", lease_expires_at)
