...
class BaseWorker:  # pylint: disable=too-many-instance-attributes
    ...
    async def _fetch_task(self):
        ...
        while self.running:
            ...
            if (
                len(self.active_tasks) >= self.args.POOL_SIZE
                or (task := self.fetch_task()) is not None
            ):
                if task is not None:
            ...
            await asyncio.sleep(random.uniform(1.0, wait_duration))
            wait_duration = max(wait_duration * 2, 60.0)
